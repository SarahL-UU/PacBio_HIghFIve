...
validate(config, schema="/proj/naiss2024-22-121/pb_longread_pipeline/PacBio_HIghFIve/workflow/schemas/config.schema.yaml")

config = load_resources(config, config["resources"])
validate(config, schema="/proj/naiss2024-22-121/pb_longread_pipeline/PacBio_HIghFIve/workflow/schemas/resources.schema.yaml")


### Read and validate samples file
samples = pandas.read_table(config["samples"], dtype=str).set_index("sample", drop=False)
validate(samples, schema="/proj/naiss2024-22-121/pb_longread_pipeline/PacBio_HIghFIve/workflow/schemas/samples.schema.yaml")


### Read and validate units file
units = (
    pandas.read_table(config["units"], dtype=str)
    .set_index(["sample", "platform"], drop=False)
    .sort_index()
)
validate(units, schema="/proj/naiss2024-22-121/pb_longread_pipeline/PacBio_HIghFIve/workflow/schemas/units.schema.yaml")
...
def generate_copy_code(workflow, output_json):
    ...
    print(code)  # Print the generated code for debugging
